...
// Dead dev routes that were deleted in the storage consolidation. They must
// stay deleted — they loaded hardcoded data (6a.json) and bypassed auth.
const DELETED_API_ROUTES = [
	"src/routes/api/+server.ts",
	"src/routes/api/results/+server.ts",
	"src/routes/api/student-files/+server.ts",
];
...
describe("Slice 10: globalFallback removal", () => {
	...
	describe("Dead dev routes stay deleted", () => {
		for (const route of DELETED_API_ROUTES) {
			it(`${route} is not in the workspace`, () => {
				expect(() => readFile(route)).toThrow();
			});
		}
	});
