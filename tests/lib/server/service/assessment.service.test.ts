...
const { mockMistralOcr, mockTenantFileStorage } = vi.hoisted(() => {
	const mockMistralOcr = {
		processDocument: vi.fn(),
		getMarkdownByFileId: vi.fn(),
	};
	const mockTenantFileStorage = {
		...
		save: vi.fn().mockResolvedValue("extracted/stored"),
		load: vi.fn().mockResolvedValue(null),
		getImage: vi.fn().mockResolvedValue(null),
		listStudentFolders: vi.fn().mockResolvedValue([]),
		savePending: vi.fn().mockResolvedValue("extracted/pending"),
		loadByStudent: vi.fn().mockResolvedValue(null),
		findLatestByStudent: vi.fn().mockResolvedValue(null),
		loadRawText: vi.fn().mockResolvedValue(null),
		deleteRawText: vi.fn().mockResolvedValue(undefined),
		deleteStudentFolder: vi.fn().mockResolvedValue(undefined),
		clearAll: vi.fn().mockResolvedValue(undefined),
	};
	return { mockMistralOcr, mockTenantFileStorage };
});
...
vi.mock("$lib/server/mastra/storage/tenant-file-storage", () => ({
	createTenantFileStorage: vi.fn().mockResolvedValue(mockTenantFileStorage),
	TenantFileStorage: vi.fn().mockImplementation(() => mockTenantFileStorage),
}));

import { AssessmentService } from "$lib/server/service/assessment.service";
import { AssessmentOcrService } from "$lib/server/service/assessment-ocr.service";
...
describe("Slice 1 — AssessmentService ScopedRepositoryProvider plumbing", () => {
	...
	describe("Slice 10 — provider is required (no globalFallback)", () => {
		...
		it("createAssessmentServiceForRequest builds a service bound to the supplied tenant", async () => {
			const { createAssessmentServiceForRequest } = await import("$lib/server/service/assessment.service");
	...
	describe("Slice 2 cleanup — AssessmentOcrService.runExtraction no longer uses EdApexGateway", () => {
		beforeEach(() => {
			mockMistralOcr.processDocument.mockReset();
			mockTenantFileStorage.saveRawText.mockClear();
			mockTenantFileStorage.save.mockClear();
		});
		...
		it("calls mistralOcrService.processDocument (not EdApexGateway.executeExtraction)", async () => {
			...
			const provider = makeProvider(2);
			const svc = new AssessmentOcrService(provider, mockTenantFileStorage as any);
		...
		it("saves the OCR text via TenantFileStorage.saveRawText", async () => {
			...
			expect(mockTenantFileStorage.saveRawText).toHaveBeenCalledWith(
				"42",
				"ocr.md",
				"# OCR result",
			);
		...
		it("does NOT import or construct EdApexGateway in the ocr service module", async () => {
