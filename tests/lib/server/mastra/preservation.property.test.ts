...
import { TOOL_MAP, resolveToolsForMessage } from '$lib/server/mastra/skill-tools';
import { coreTools, workflowTools, searchEntityTool } from '$lib/server/mastra/tools/index';
...
import { supervisorAgent } from '$lib/server/mastra/agents/supervisor';
import { assistantAgent } from '$lib/server/mastra/agents/assistant';
...
async function createTestGateway(): Promise<EdApexGateway> {
	...
	// Load the skill registry from the module-level singleton
	const { skillRegistry } = await import('$lib/server/mastra/skill-tools');
	const registry = skillRegistry as SkillRegistry;
...
describe('Property 2: Preservation — Behavioral Contract Integrity', () => {
	...
	describe('2.1 Supervisor Instructions contain correct TenantContext IDs and delegation strategy', () => {
		it('for all TenantContext values, supervisor instructions contain the correct IDs', async () => {
			await fc.assert(
				fc.asyncProperty(arbTenantContext, async (context) => {
					const rc = new RequestContext();
					rc.set('tenantContext', context);
					const raw: any = await (supervisorAgent as any).getInstructions({ requestContext: rc });
					const flat = typeof raw === 'string' ? raw : JSON.stringify(raw);

					// Must contain all TenantContext IDs (substring match tolerates the `- ` prefix used in actual instructions)
					expect(flat).toContain(`School ID: ${context.schoolId}`);
					expect(flat).toContain(`User ID: ${context.userId}`);
					expect(flat).toContain(`Designation ID: ${context.designationId}`);
					expect(flat).toMatch(
						new RegExp(`Class ID:\\s*${context.classId || 'None'}`)
					);
					expect(flat).toMatch(
						new RegExp(`Section ID:\\s*${context.sectionId || 'None'}`)
					);
					expect(flat).toMatch(
						new RegExp(`Exam ID:\\s*${context.examId || 'None'}`)
					);

					// Must contain delegation strategy
					expect(flat).toContain('DELEGATION STRATEGY');
					expect(flat).toContain('assistant');

					// Must contain confidence gate
					expect(flat).toContain('CONFIDENCE GATE');
	...
	describe('2.2 Slash command tool resolution produces the same tool set as resolveToolsForMessage()', () => {
		it('for all slash commands, tool resolution always includes global tools', () => {
			fc.assert(
				fc.property(arbSlashCommand, (command) => {
					const resolvedTools = resolveToolsForMessage(
						`${command} test input`,
						true
					);
		...
		it('for all slash commands, resolveToolsForMessage is deterministic (same input → same output)', () => {
			fc.assert(
				fc.property(arbSlashCommand, (command) => {
					const msg = `${command} test input`;

					const result1 = resolveToolsForMessage(msg, true);
					const result2 = resolveToolsForMessage(msg, true);
		...
		it('workflow commands (/extract, /generate, /validate, /publish) include workflow tools', () => {
			...
			for (const command of workflowCommands) {
				...
				// Workflow tools must be present (keyed by tool id, e.g. extract-document)
				expect(toolKeys).toContain('extract-document');
				expect(toolKeys).toContain('validate-extraction');
				expect(toolKeys).toContain('publish-results');
			}
		});
	});
	...
	describe('2.3 Non-slash messages return the full core + workflow + global tool set', () => {
		it('for all non-slash messages, tool resolution returns core + workflow + global tools', () => {
			...
			const expectedTools = resolveToolsForMessage(
				'Hello there',
				false
			);
	...
	describe('2.5 Assistant instructions contain tenant boundaries and behavioral guidelines', () => {
		it('for all TenantContext values, assistant instructions contain correct IDs and guidelines', async () => {
			await fc.assert(
				fc.asyncProperty(arbTenantContext, async (context) => {
					const rc = new RequestContext();
					rc.set('tenantContext', context);
					const instructions: any = await (assistantAgent as any).getInstructions({ requestContext: rc });
					const flat = typeof instructions === 'string' ? instructions : JSON.stringify(instructions);

					// Must contain tenant boundary IDs (substring match tolerates the `- ` prefix)
					expect(flat).toContain(`School ID: ${context.schoolId}`);
					expect(flat).toContain(`User ID: ${context.userId}`);
					expect(flat).toMatch(
						new RegExp(`Class ID:\\s*${context.classId || 'None'}`)
