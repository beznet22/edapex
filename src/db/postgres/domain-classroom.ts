 ...
 */
import { pgSchema, text, doublePrecision, integer, uuid, timestamp, jsonb, varchar, index } from "drizzle-orm/pg-core";
...
export const classroomSessions = classroomSchema.table("sessions", {
...
export const classroomMemoryLedger = classroomSchema.table("memory_ledger", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: uuid("tenant_id").notNull().references(() => tenants.id),
  sessionId: uuid("session_id").notNull().references(() => classroomSessions.id),
...
export const classroomParticipants = classroomSchema.table("participants", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: uuid("tenant_id").notNull().references(() => tenants.id),
  sessionId: uuid("session_id").notNull().references(() => classroomSessions.id),
...
export const classroomWhiteboardState = classroomSchema.table("whiteboard_state", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: uuid("tenant_id").notNull().references(() => tenants.id),
  sessionId: uuid("session_id").notNull().references(() => classroomSessions.id),
