...
export const feeAssignments = financeSchema.table(
  "fee_assignments",
  {
    id: uuid("id")
      .primaryKey()
...
export const feeInstallments = financeSchema.table(
  "fee_installments",
  {
    id: uuid("id")
      .primaryKey()
...
export const financeEvents = financeSchema.table(
  "finance_events",
  {
    id: uuid("id")
      .primaryKey()
    ...
    category: varchar("category", { length: 30 }).notNull(), // 'ai_cost' | 'tuition' | 'payroll' | 'operational' | 'refund' | 'adjustment'
    ...
    metadata: jsonb("metadata").$type<FinanceEventMetadata>(),
    postedAt: timestamp("posted_at").defaultNow(),
