...
export const feeMasters = mysqlTable("fee_masters", {
  ...
  amount: decimal("amount", { precision: 12, scale: 2 }).notNull(),
  academicId: varchar("academic_id", { length: 36 })
    .notNull()
...
export const feeAssignments = mysqlTable(
  "fee_assignments",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
    ...
    tenantId: varchar("tenant_id", { length: 36 })
      .notNull()
      .references(() => tenants.id),
    feeMasterId: varchar("fee_master_id", { length: 36 })
      .notNull()
    ...
    status: mysqlEnum("status", ["pending", "partial", "paid", "overdue", "waived"])
      .notNull()
      .default("pending"),
    academicId: varchar("academic_id", { length: 36 })
      .notNull()
...
export const feeInstallments = mysqlTable(
  "fee_installments",
  {
    ...
    tenantId: varchar("tenant_id", { length: 36 })
      .notNull()
      .references(() => tenants.id),
    feeAssignmentId: varchar("fee_assignment_id", { length: 36 })
      .notNull()
...
export const invoices = mysqlTable(
  "invoices",
  {
    ...
    paidAmount: decimal("paid_amount", { precision: 12, scale: 2 }).default("0.00"),
    status: mysqlEnum("status", ["draft", "issued", "paid", "partial", "overdue", "cancelled"])
      .notNull()
      .default("draft"),
...
export const financeEvents = mysqlTable(
  "finance_events",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
    ...
    metadata: json("metadata").$type<FinanceEventMetadata>(),
    postedAt: timestamp("posted_at").defaultNow(),
