...
export const aiTasks = sqliteTable(
  "ai_tasks",
  {
    ...
    tenantId: text("tenant_id", { length: 36 })
      .notNull()
      .references(() => tenants.id),
    sessionId: text("session_id", { length: 36 }).references(() => aiSessions.id), // [CONTROL PLANE] Lineage to Session
    goalId: text("goal_id", { length: 36 }),
    title: text("title", { length: 255 }).notNull(),
    description: text("description"),
    status: text("status", {
      enum: ["backlog", "todo", "in_progress", "completed", "failed", "cancelled", "blocked"],
    }).default("todo"),
...
export const aiToolInvocations = sqliteTable(
  "ai_tool_invocations",
  {
    ...
    tenantId: text("tenant_id", { length: 36 })
      .notNull()
      .references(() => tenants.id),
    actionId: text("action_id", { length: 36 })
      .notNull()
