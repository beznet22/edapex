    ...
    <div class="flex flex-col gap-4 w-full">
        ...
        {#if store.activeTab === "results"}
            <Button
                ...
                disabled={!store.extractedData || store.isModalLoading}
            >
        ...
        <div class="grid grid-cols-3 bg-muted/50 p-1 sm:p-1.5 h-10 sm:h-11 rounded-xl sm:rounded-2xl shadow-inner max-w-md mx-auto w-full">
            <button
                class="rounded-lg sm:rounded-xl text-[8px] sm:text-[9px] font-black uppercase tracking-widest transition-all duration-200 {store.activeTab === 'viewer' ? 'bg-background shadow-lg text-foreground' : 'text-muted-foreground hover:text-foreground/70'}"
                onclick={() => (store.activeTab = "viewer")}
            >
                Viewer
            </button>
            <button
                class="rounded-lg sm:rounded-xl text-[8px] sm:text-[9px] font-black uppercase tracking-widest transition-all duration-200 {store.activeTab === 'results' ? 'bg-background shadow-lg text-foreground' : 'text-muted-foreground hover:text-foreground/70'}"
                onclick={() => (store.activeTab = "results")}
            >
                Results
            </button>
            <button
                class="rounded-lg sm:rounded-xl text-[8px] sm:text-[9px] font-black uppercase tracking-widest transition-all duration-200 {store.activeTab === 'json' ? 'bg-background shadow-lg text-foreground' : 'text-muted-foreground hover:text-foreground/70'}"
            ...
            <div class="flex-1 relative sm:overflow-hidden flex-col {store.activeTab === 'viewer' ? 'flex' : 'hidden'}">
                <div class="flex-1 relative flex flex-col bg-neutral-950 sm:bg-transparent sm:overflow-hidden">
                    ...
                    <div class="absolute top-6 left-1/2 -translate-x-1/2 flex items-center gap-1.5 px-4 py-2 bg-background/20 backdrop-blur-2xl rounded-2xl border border-white/10 z-20 shadow-2xl">
                        ...
                        <span class="text-[11px] text-white font-black w-10 text-center tabular-nums">
                    ...
                    <div
                        ...
                        onwheel={(e) => {
                            if (e.ctrlKey) {
                                e.preventDefault();
                                store.zoom = Math.min(Math.max(store.zoom + (e.deltaY < 0 ? 0.1 : -0.1), 0.5), 3);
                            }
                        }}
                    >
                        {#if store.selectedFile?.url}
                            ...
                            >
                                ...
                                />
                                {#if store.showGrids}
                                    <div class="absolute inset-0 grid grid-cols-12 grid-rows-12 pointer-events-none opacity-30 mix-blend-overlay">
                                        {#each Array(144) as _}
                                            <div class="border-[0.5px] border-primary/50"></div>
                                        {/each}
                                    </div>
                                {/if}
                            </div>
                        {:else}
                            <div class="flex flex-col items-center justify-center text-muted-foreground/20 animate-pulse">
                                <FileText class="h-32 w-32 mb-6" />
                                <p class="text-xs font-black uppercase tracking-[0.4em]">Empty Canvas</p>
                            </div>
                        {/if}
                    </div>
                    ...
                    <div class="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3 z-20">
                        <Button
                            variant={store.showGrids ? "default" : "secondary"}
                            size="sm"
                            class="h-10 px-5 rounded-xl shadow-2xl transition-all {store.showGrids ? 'bg-primary' : 'bg-background/20 backdrop-blur-xl text-white border border-white/10'}"
                            onclick={() => (store.showGrids = !store.showGrids)}
                        >
                            <Grid3X3 class="w-4 h-4 mr-2" />
                            <span class="text-[9px] font-black uppercase tracking-widest">Grids</span>
                        </Button>
                        <Button
                            variant={store.showTranslation ? "default" : "secondary"}
                            size="sm"
                            class="h-10 px-5 rounded-xl shadow-2xl transition-all {store.showTranslation ? 'bg-primary' : 'bg-background/20 backdrop-blur-xl text-white border border-white/10'}"
                            onclick={() => (store.showTranslation = !store.showTranslation)}
                        >
                            <Languages class="w-4 h-4 mr-2" />
                            <span class="text-[9px] font-black uppercase tracking-widest">Translate</span>
                        </Button>
                    </div>
                </div>
            </div>
                            ...
                            {:else if store.extractedData}
                                ...
                                {#if data}
                                    <div class="space-y-10">
                                        ...
                                        <div class="space-y-6">
                                            <div class="flex items-center gap-4">
                                                <div class="w-1.5 h-6 bg-primary rounded-full shadow-[0_0_15px_rgba(var(--primary),0.5)]"></div>
                                                <h3 class="text-xs font-black uppercase tracking-[0.3em] text-foreground/80">
                                                    Extraction Context
                                                </h3>
                                            </div>
                                            <div class="grid gap-4 bg-muted/20 p-5 rounded-3xl border border-border/50">
                                                <div class="space-y-2">
                                                    <Label class="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Full Name</Label>
                                                    {#if data.studentData}
                                                        <Input bind:value={data.studentData.fullName} class="h-11 bg-background border-none shadow-sm font-black text-sm rounded-2xl px-4 uppercase" />
                                                    {:else}
                                                        <Input bind:value={data.fullName} class="h-11 bg-background border-none shadow-sm font-black text-sm rounded-2xl px-4 uppercase" />
                                                    {/if}
                                                </div>
                                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                    <div class="space-y-2">
                                                        <Label class="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Admission No</Label>
                                                        {#if data.studentData}
                                                            <Input type="number" bind:value={data.studentData.admissionNo} class="h-11 bg-background border-none shadow-sm font-black text-sm rounded-2xl px-4" />
                                                        {/if}
                                                    </div>
                                                    <div class="space-y-2">
                                                        <Label class="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Category</Label>
                                                        {#if data.studentData}
                                                            <Input bind:value={data.studentData.studentCategory} class="h-11 bg-background border-none shadow-sm font-black text-sm rounded-2xl px-4 uppercase" />
                                                        {/if}
                                                    </div>
                                                </div>
                                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                    <div class="space-y-2">
                                                        ...
                                                        {#if data.studentData}
                                                            <Input bind:value={data.studentData.term} class="h-11 bg-background border-none shadow-sm font-black text-sm rounded-2xl px-4 uppercase" />
                                                        {/if}
                                                    </div>
                                                    <div class="space-y-2">
                                                        <Label class="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Extracted At</Label>
                                                        <div class="h-11 flex items-center bg-background border-none shadow-sm font-black text-[10px] rounded-2xl px-4 uppercase truncate">
                                                            {store.extractedData.extractedAt ? new Date(store.extractedData.extractedAt).toLocaleDateString() : "N/A"}
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                    <div class="space-y-2">
                                                        <Label class="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Verified</Label>
                                                        <div class="h-11 flex items-center bg-background border-none shadow-sm font-black text-xs rounded-2xl px-4 uppercase {store.extractedData.verified ? 'text-emerald-500' : 'text-amber-500'}">
                                                            {store.extractedData.verified ? "YES" : "NO"}
                                                        </div>
                                                    </div>
                                                    <div class="space-y-2">
                                                        <Label class="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Status</Label>
                                                ...
                                                {#if store.extractedData.error}
                                                    <div class="p-4 bg-destructive/10 border border-destructive/20 rounded-2xl">
                                                        <p class="text-[10px] font-bold text-destructive uppercase tracking-tight leading-relaxed">
                                                            {store.extractedData.error}
                                                        </p>
                                                    </div>
                                                {/if}
                                            </div>
                                        ...
                                        {#if data.studentData?.attendance}
                                            <div class="space-y-6">
                                                ...
                                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-muted/20 p-5 rounded-3xl border border-border/50">
                                                    <div class="space-y-2">
                                                        <Label class="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Opened</Label>
                                                        <Input type="number" bind:value={data.studentData.attendance.daysOpened} class="h-11 bg-background border-none shadow-sm font-black text-sm rounded-2xl px-4" />
                                                    </div>
                                                    <div class="space-y-2">
                                                        <Label class="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Present</Label>
                                                        <Input type="number" bind:value={data.studentData.attendance.daysPresent} class="h-11 bg-background border-none shadow-sm font-black text-sm rounded-2xl px-4" />
                                                    </div>
                                                    <div class="space-y-2">
                                                        <Label class="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Absent</Label>
                                                        <Input type="number" bind:value={data.studentData.attendance.daysAbsent} class="h-11 bg-background border-none shadow-sm font-black text-sm rounded-2xl px-4" />
                                                    </div>
                                                </div>
                                            </div>
                                        {/if}
                                        ...
                                        <div class="space-y-6">
                                            ...
                                            <div class="rounded-4xl border border-border/50 bg-background shadow-xl overflow-x-auto scrollbar-hide">
                                                <table class="w-full text-left">
                                                    <thead>
                                                        <tr class="bg-muted/30 border-b border-border/50">
                                                            <th class="p-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Subject</th>
                                                            <th class="p-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 text-center">Marks</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody class="divide-y divide-border/30">
                                                                ...
                                                                <tr class="hover:bg-muted/5 transition-colors group">
                                                                    <td class="p-4">
                                                                        <div class="flex flex-col">
                                                                            <span class="text-sm font-black uppercase tracking-tight">{m.subjectName || m.subjectCode}</span>
                                                                            <span class="text-xs font-medium text-muted-foreground opacity-50 uppercase tracking-widest">{m.subjectCode}</span>
                                                                        </div>
                                                                    </td>
                                                                    <td class="p-4">
                                                                            ...
                                                                            <div class="flex gap-2 justify-center">
                                                                                {#each m.marks as score, idx}
                                                                                    <input
                                                                                        type="number"
                                                                                        value={score}
                                                                                        oninput={(e) => store.updateScore(m.subjectCode, idx, e.currentTarget.value)}
                                                        ...
                                                        {:else if data.scores}
                                                            {#each Object.entries(data.scores) as [subject, marks]}
                                                                <tr class="hover:bg-muted/5 transition-colors group">
                                                                    <td class="p-4">
                                                                        <span class="text-sm font-black uppercase tracking-tight">{subject}</span>
                                                                    </td>
                                                                    <td class="p-4">
                                                                        <div class="flex gap-2 justify-center">
                                                                            {#each Array.isArray(marks) ? marks : [marks] as score, idx}
                                                                                <input
                                                                                    type="number"
                                                                                    value={score}
                                                                                    oninput={(e) => store.updateScore(subject, idx, e.currentTarget.value)}
                                                                                    class="w-11 h-9 text-center bg-muted/40 border-none rounded-xl text-sm font-black focus:bg-background focus:ring-4 focus:ring-primary/10 transition-all outline-none"
                                                                                />
                                                                            {/each}
                                                                        </div>
                                        ...
                                        {#if data.studentRatings}
                                            <div class="space-y-6">
                                                <div class="flex items-center gap-4">
                                                    <div class="w-1.5 h-6 bg-purple-500 rounded-full shadow-[0_0_15px_rgba(168,85,247,0.5)]"></div>
                                                    <h3 class="text-xs font-black uppercase tracking-[0.3em] text-foreground/80">
                                                        Student Ratings
                                                    </h3>
                                                </div>
                                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                    {#each Object.entries(data.studentRatings) as [key, value]}
                                                        <div class="space-y-2 bg-muted/20 p-4 rounded-3xl border border-border/50">
                                                            <Label class="text-[11px] font-black uppercase tracking-widest text-muted-foreground ml-1">
                                                                {key.replace(/_/g, " ")}
                                                            </Label>
                                                            <Select.Root
                                                                type="single"
                                                                value={String(value || "")}
                                                                onValueChange={(v) => { if (v) data.studentRatings[key] = Number(v); }}
                                                            >
                                        ...
                                        {#if data.teachersRemark}
                                            <div class="space-y-6">
                                                ...
                                                <div class="rounded-3xl border border-border/50 bg-background shadow-xl p-1">
                                                    <Textarea
                                                        bind:value={data.teachersRemark.comment}
                                ...
                                {/if}
                            {:else}
                                <div class="flex flex-col items-center justify-center py-40 text-muted-foreground/30 text-center space-y-6">
                                    <div class="p-8 rounded-[2.5rem] bg-muted/10 border border-dashed border-muted/50">
                                        <Search class="w-12 h-12 opacity-20" />
                                    </div>
                                    <p class="text-[10px] font-black uppercase tracking-[0.3em]">No Analysis Data Found</p>
                                </div>
                            {/if}
                        </div>
                    {/if}
