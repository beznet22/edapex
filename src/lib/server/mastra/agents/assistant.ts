	...
	instructions: ({ requestContext }) => {
		...
		if (ctx) {
			instructions.push(
				...
				'- OCR cannot link marksheet images to DB students. The OCR returns whatever text is on the page; only @mentions in the user message OR a manual `link-marksheet-student` call at HITL can resolve identity.',
				...
				'or similar. You MUST call the `stream-document` tool to transform the raw OCR markdown into',
