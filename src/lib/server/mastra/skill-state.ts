 ...
 */
const LOCK_BYPASS_TOOLS = new Set(["manage-access", "system-status"]);
...
export class SkillStateManager {
  ...
  async activateSkill(sessionId: string, skillName: string, locked: boolean): Promise<void> {
   ...
   */
  async switchSkill(sessionId: string, newSkillName: string, isLocked: boolean): Promise<SwitchResult> {
   ...
   */
  evaluateInterruption(sessionId: string, detectedSkill: string): InterruptionResult {
    const current = this.sessions.get(sessionId);
