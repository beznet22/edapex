...
4. **Pre-conditions**: Resolve the target `studentId` via `search-entity` or `@mention` before invoking tools. Reject with `ForbiddenError` if the caller lacks designation {1, 5, 8}.
