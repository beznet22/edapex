...
2. **Context Discovery**: Use `search-school-directory` to resolve `@mentions` and lookups before routing to a domain skill.
...
- **Onboarding**: `/enroll`, `/admit`, `/transfer`
- **Gov**: `/update`, `/suspend`, `/delete`, `/password`
- **Assistant**: `/extract`, `/validate`, `/publish`, `/generate`
- **Default**: `/search`, `/switch`, `/context`
