...
async function generate(admissionNo: number, locals: App.Locals, cookies: { get: (key: string) => string | undefined }) {
  ...
  if (typeof executeFn !== "function") throw new Error("generateResultPdfTool.execute is not bound");
...
export const POST: RequestHandler = async ({ request, cookies, locals }) => {
  ...
  return json(await generate(admissionNo, locals, cookies));
};
