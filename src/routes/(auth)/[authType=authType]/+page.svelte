...
<div class="fixed inset-0 -z-10 overflow-hidden bg-background">
  <!-- Static Glows (Subtle) -->
  <div
    class="absolute top-[-20%] left-[-10%] w-[70%] h-[70%] bg-primary/7 rounded-full blur-[120px]"
  ></div>
  <div
    class="absolute bottom-[-20%] right-[-10%] w-[70%] h-[70%] bg-primary/3 rounded-full blur-[120px]"
  ></div>
...
>
  ...
  >
    <div
      class="flex flex-col gap-8 rounded-[28px] bg-card/30 p-4 sm:p-10 border border-border/10"
    >
      <div class="flex flex-col items-center justify-center gap-3 text-center">
        <div
          class="size-12 rounded-2xl bg-primary/10 flex items-center justify-center border border-primary/20 mb-2"
        >
          <div
            class="size-6 rounded-lg bg-primary shadow-[0_0_20px_rgba(var(--primary),0.5)]"
          ></div>
        </div>
        <h3
          class="text-2xl font-black uppercase tracking-tighter text-foreground"
        >
          {signInSignUp}
        </h3>
        <p
          class="text-[11px] font-bold uppercase tracking-widest text-muted-foreground/60 max-w-[240px] leading-relaxed"
        >
      ...
      {#if page.params.authType === "signup"}
        <div class="flex flex-col gap-6 w-full">
          <p
            class="text-center text-[11px] font-bold uppercase tracking-widest text-muted-foreground"
          >
            Account creation is temporarily disabled. <br /> Please contact your
            administrator.
          </p>
...
{#snippet switchAuthType({
  ...
  <p
    class="mt-6 text-center text-[11px] font-bold uppercase tracking-widest text-muted-foreground/60"
  >
    {question}
    <a {href} class="text-primary hover:text-primary/80 transition-colors ml-1">
      {cta}
    </a>
    {postscript}
  </p>
