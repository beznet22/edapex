...
import { 
  ...
  complaints, 
  visitors 
} from "../../../db/postgres/domain-facilities.js";
import { 
...
export class PostgresFacilitiesRepository implements IFacilitiesRepository {
  // --- Dormitories ---
  ...
  // --- Operations ---
  async getComplaints(tenantId: string, filter?: { status?: string; userId?: string }): Promise<IComplaint[]> {
    let whereClause = eq(complaints.tenantId, tenantId);
    if (filter?.userId) {
      whereClause = and(whereClause, eq(complaints.complaintBy, filter.userId)) as any;
    }
    if (filter?.status) {
      whereClause = and(whereClause, eq(complaints.status, filter.status as any)) as any;
    }
    const results = await db.select().from(complaints).where(whereClause);
    return results.map((row: any) => ({ ...row }));
  }
