...
import { 
  attendances 
} from "../../../db/mysql/domain-attendance.js";
import { 
...
export class MySqlAttendanceRepository implements IAttendanceRepository {
  ...
  private mapStaffAttendance(row: any): IStaffAttendance {
    return {
      ...row,
      staffId: row.userId, // In this schema userId is the participant
      status: this.mapStatus(row.status),
      note: row.metadata?.notes || null,
      createdAt: row.createdAt ? new Date(row.createdAt) : null,
      updatedAt: row.updatedAt ? new Date(row.updatedAt) : null,
  ...
  async bulkSaveStudentAttendance(data: Partial<IStudentAttendance>[]): Promise<void> {
    ...
    // Simple implementation: delete and insert for daily attendance to handle updates
    for (const val of values) {
      await db.insert(attendances).values(val as any).onDuplicateKeyUpdate({
  ...
  async saveStaffAttendance(data: Partial<IStaffAttendance>): Promise<IStaffAttendance> {
    ...
    const [updated] = await db.select().from(attendances).where(and(
      eq(attendances.userId, data.staffId!),
      eq(attendances.attendanceDate, data.attendanceDate!)
    ));
    return this.mapStaffAttendance(updated);
  }
