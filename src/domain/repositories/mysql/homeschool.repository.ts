...
export class MySqlHomeschoolRepository implements IHomeschoolRepository {
  ...
  async createSubscription(data: Partial<IHomeschoolSubscription>): Promise<IHomeschoolSubscription> {
    const [result] = await db.insert(homeschoolSubscriptions).values(data as any);
    const [row] = await db.select().from(homeschoolSubscriptions).where(eq(homeschoolSubscriptions.id, result.insertId));
  ...
  async createSchedule(data: Partial<IHomeschoolSchedule>): Promise<IHomeschoolSchedule> {
    const [result] = await db.insert(homeschoolSchedules).values(data as any);
    const [row] = await db.select().from(homeschoolSchedules).where(eq(homeschoolSchedules.id, result.insertId));
  ...
  async createPortfolio(data: Partial<IHomeschoolPortfolio>): Promise<IHomeschoolPortfolio> {
    const [result] = await db.insert(homeschoolPortfolios).values(data as any);
    const [row] = await db.select().from(homeschoolPortfolios).where(eq(homeschoolPortfolios.id, result.insertId));
